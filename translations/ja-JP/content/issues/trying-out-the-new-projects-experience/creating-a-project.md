...
- 繰り返し: 値は日付範囲のセット（繰り返し）から選択しなければなりません。 過去の繰り返しは、自動的に"completed"としてマークされ、現在の日付範囲をカバーする繰り返しは"current"としてマークされます。
...
2. 繰り返しフィールドでは、繰り返しの追加や削除、繰り返し名の変更、繰り返しの開始日と期間を変更できます。
